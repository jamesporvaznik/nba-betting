"""
fetch_team_defensive_playtype_stats.py
------------------------------
Fetches team defensive play type data using nba_api 
...
def fetch_playtype(season: str, play_type: str) -> pd.DataFrame:
    """Fetch team defensive data for a specific play type."""
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if df is not None and not df.empty:
                log.info(f"    ✓ {play_type}: {len(df)} teams")
                return df
            else:
...
def transform_data(df: pd.DataFrame, season: str) -> pd.DataFrame:
    """Clean and transform the data for TEAMS defensive stats."""
    transformed = pd.DataFrame()
    
    # Team data
    transformed['team_id'] = df.get('TEAM_ID', 0)
    transformed['team_abbreviation'] = df.get('TEAM_ABBREVIATION', 'Unknown')
    transformed['season'] = season
    transformed['play_type'] = df.get('PLAY_TYPE', 'Unknown')
    
    # Core stats - what the defense allows
    transformed['possessions_defended'] = pd.to_numeric(df.get('POSS', 0), errors='coerce').fillna(0).round(2)
    transformed['points_allowed'] = pd.to_numeric(df.get('PTS', 0), errors='coerce').fillna(0).round(2)
    ...
    # Efficiency allowed
    transformed['points_allowed_per_possession'] = transformed.apply(
        lambda row: round(row['points_allowed'] / row['possessions_defended'], 3) if row['possessions_defended'] > 0 else 0,
        axis=1
    )
    transformed['fg_pct_allowed'] = pd.to_numeric(df.get('FG_PCT', 0), errors='coerce').fillna(0).round(4)
    transformed['efg_pct_allowed'] = pd.to_numeric(df.get('EFG_PCT', 0), errors='coerce').fillna(0).round(4)
...
def upsert_batch(supabase_client: Client, batch: list, table_name: str, total_upserted: int) -> int:
    ...
    try:
        result = supabase_client.table(table_name).upsert(
            batch, on_conflict="team_id,season,play_type"
        ).execute()
...
def main():
    log.info("="*80)
    log.info("🏀 FETCHING TEAM DEFENSIVE PLAY TYPE STATS & UPSERTING TO SUPABASE")
    log.info("="*80)
    ...
    final_df = final_df.drop_duplicates(subset=['team_id', 'season', 'play_type'])
    ...
    batch = []
    total_upserted = 0
    table_name = "team_defensive_playtype_stats"
