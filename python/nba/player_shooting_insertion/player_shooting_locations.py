"""
fetch_player_shot_locations.py
------------------------------
Fetches player shot location data (zone, area, etc.) from NBA API
...
def fetch_shot_locations(season: str) -> pd.DataFrame:
    """Fetch shot location data for a specific season."""
...
def fetch_season_data(season: str) -> pd.DataFrame:
    """Fetch shot location data for a season."""
    log.info(f"\n{'='*50}")
    log.info(f"Processing {season}")
    log.info('='*50)
    log.info(f"  Fetching shot locations...")
...
def transform_data(df: pd.DataFrame, season: str) -> pd.DataFrame:
    """Clean and transform the shot location data into a normalized format."""
    
    # First, extract the player info (these columns are consistent)
    player_id_col = next((col for col in df.columns if 'PLAYER_ID' in col), None)
    
    if not player_id_col:
        log.error("    Could not find player ID column")
        return pd.DataFrame()
    
    # Create a list to hold all zone data
    all_zones = []
    
    # For each player, extract data for each shot zone
    for idx, row in df.iterrows():
        player_id = row[player_id_col]
        
        for zone in SHOT_ZONES:
            # Look for columns that match this zone
            ...
            if fgm_col and fga_col:
                ...
                # Only include if the player actually took shots from this zone
                if fga > 0:
                    all_zones.append({
                        'player_id': player_id,
                        'season': season,
                        'shot_zone': zone,
                        'fgm': round(float(fgm), 2),
                        'fga': round(float(fga), 2),
                        'fg_pct': round(float(fg_pct), 4)
                    })
    ...
    # Calculate frequency percentage per player (what % of their shots come from each zone)
    player_totals = transformed.groupby('player_id')['fga'].sum().reset_index()
    player_totals.columns = ['player_id', 'total_fga']
    
    transformed = transformed.merge(player_totals, on='player_id')
    transformed['frequency_pct'] = round(transformed['fga'] / transformed['total_fga'], 4)
    transformed = transformed.drop('total_fga', axis=1)
...
def upsert_batch(supabase_client: Client, batch: list, table_name: str, total_upserted: int) -> int:
    ...
    try:
        result = supabase_client.table(table_name).upsert(
            batch, on_conflict="player_id,season,shot_zone"
        ).execute()
...
def main():
    log.info("="*80)
    log.info("🏀 FETCHING PLAYER SHOT LOCATION STATS & UPSERTING TO SUPABASE")
    log.info("="*80)
    ...
    for season in SEASONS:
        ...
        if not raw_df.empty:
            ...
            if not transformed_df.empty:
                ...
                log.info(f"    Unique players: {transformed_df['player_id'].nunique()}")
    ...
    final_df = final_df.drop_duplicates(subset=['player_id', 'season', 'shot_zone'])
    ...
    print("\n📊 Shot zones collected:")
    for zone in final_df['shot_zone'].unique():
        zone_data = final_df[final_df['shot_zone'] == zone]
        print(f"  {zone}: {len(zone_data)} entries, {zone_data['player_id'].nunique()} players")
